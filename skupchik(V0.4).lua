...
function sampev.onServerMessage(color, text)
	if onserversessage.v then
		if text:find('Это нельзя сделать находясь в интерьере!') then
			return {-1, tag .. "Это нельзя сделать находясь в интерьере!"}
		elseif text:find('Лавка не может быть установлена рядом со входом в бизнесы!') then
			return {-1, tag .. "Лавка не может быть установлена рядом со входом в бизнесы!"}
		elseif text:find('Лавка не может быть установлена рядом с другими лавками (меньше 5 метров)!') then
			return {-1, tag .. "Лавка не может быть установлена рядом с другими лавками (меньше 5 метров)!"} 
		elseif text:find('Лавка не может быть установлена рядом со входом в дом!') then
			return {-1, tag .. "Лавка не может быть установлена рядом со входом в дом!"} 
		elseif text:find('Ваша лавка была закрыта, из-за того что вы её покинули!') then
			return {-1, tag .. "Ваша лавка была закрыта, из-за того что вы её покинули!"}
		elseif text:find('Ваша лавка была закрыта, из-за того что вы её покинули!') then
			return {-1, tag .. "Ваша лавка была закрыта, из-за того что вы её покинули!"}
		elseif text:find('Лавка не может быть установлена рядом с другими игроками!') then
			return {-1, tag .. "Лавка не может быть установлена рядом с другими игроками!"}
		elseif text:find('Используйте клавишу: ALT для управления товарами!') then
			return {-1, tag .. "Используйте клавишу: ALT для управления товарами!"}
		elseif text:find('Вы успешно выставили лавку для продажи/покупки товара!') then
			return {-1, tag .. "Вы успешно выставили лавку для продажи/покупки товара!"}
		end
	end
end
...
function imgui.OnDrawFrame()
    ...
    if main_window_state.v then 
        ...
        imgui.Begin(u8'SkupChik for Arizona RP | Хелпер для барыг | By Mayako', main_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.AlwaysAutoResize)
		...
		elseif selected == 3 then
			...
			for i=1, #binds do
				imgui.BeginChild("##"..i, imgui.ImVec2(720, 85))
					...
					imgui.InputText(u8"Текст для биндер №"..i, buf[i])
					imgui.PopItemWidth()
					imgui.PushItemWidth(442)
					imgui.SliderFloat(u8"Задержка биндера", floats[i], 1, 150, 0, 1)
					imgui.PopItemWidth()
					binds[i][3] = floats[i].v
					if binds[i][1] == 0 then
						if imgui.Button(fa.ICON_FA_PLAY .. u8" Начать пиар",imgui.ImVec2(150, 30)) then 
							binds[i][1] = 1 piarStart(i)
						end
						else if imgui.Button(u8"Закончить пиар",imgui.ImVec2(150, 30)) then 
							binds[i][1] = 0 
						end
					end	
					imgui.SameLine()
					imgui.SetCursorPosX(385)
					if imgui.Button(fa.ICON_FA_BAN .. u8" Удалить биндер номер №"..i,imgui.ImVec2(180, 30)) then
						deleteBind(i)
						sampAddChatMessage(tag .. 'Биндер успешно удалён!',-1)
					end
				imgui.EndChild()
				imgui.Separator()
			end
			imgui.SetCursorPosX(200)
			if imgui.Button(fa.ICON_FA_SAVE .. u8"  Сохранить все настройки", imgui.ImVec2(190, 30)) then
				save()
				print(binds[1][3])
				sampAddChatMessage(tag .. 'Настройки успешно сохранены!',-1)
			end
			imgui.SetCursorPosX(200)
			if imgui.Button(fa.ICON_FA_PLUS .. u8"  Новый бинд", imgui.ImVec2(190, 30)) then
				createNewBind()
				sampAddChatMessage(tag .. 'Cоздан новый биндер!',-1)
			end
			imgui.EndChild()	
		...
		if main_window_state_2.v then
			...
			imgui.InputInt(u8"Задержка", delskup)
			imgui.PopItemWidth()
			imgui.Checkbox(u8"Замена серверных подсказок", onserversessage)
			if imgui.Button(fa.ICON_FA_SAVE .. u8' Сохранить настройки',imgui.ImVec2(250, 20)) then
				mainIni.config.delayskup = delskup.v
				mainIni.config.onserversessage = onserversessage.v
				saveIniFile()
				sampAddChatMessage(tag .. 'Настройки успешно сохранены!',-1)
			end
			if imgui.Button(fa.ICON_FA_ANGLE_DOUBLE_LEFT .. u8' Назад',imgui.ImVec2(60, 20)) then
				main_window_state_2.v = not main_window_state_2.v 
			end
			imgui.EndChild()
			imgui.End()
...
function autoupdate(json_url, prefix, url)
	sampAddChatMessage((tag .. 'Проверка обновлений...'), -1)
	  ...
	  function(id, status, p1, p2)
		if status == dlstatus.STATUSEX_ENDDOWNLOAD then
		  if doesFileExist(json) then
			...
			sampAddChatMessage((tag .. 'Не могу проверить обновление. Смиритесь или проверьте самостоятельно на.'..url), -1)
			update = false
		  end
		end
	  end
